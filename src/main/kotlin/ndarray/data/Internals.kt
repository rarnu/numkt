...
inline fun checkBounds(value: Boolean, index: Int, axis: Int, size: Int) {
    if (!value) throw IndexOutOfBoundsException("Index $index is out of bounds shape dimension $axis with size $size")
}
...
inline fun requireDimension(dim: Dimension, shapeSize: Int) {
    require(dim.d == shapeSize || (dim.d > 4 && shapeSize > 4)) { "Dimension doesn't match the size of the shape: dimension (${dim.d}) != $shapeSize shape size." }
}
 ...
 */
inline fun requireArraySizes(rightSize: Int, otherSize: Int) {
 ...
 */
inline fun requireEqualShape(left: IntArray, right: IntArray) {
 ...
 */
inline fun requirePositiveShape(dim: Int) {
 ...
 */
fun computeStrides(shape: IntArray): IntArray = shape.copyOf().apply {
...
operator fun <T : Number> Number.compareTo(other: T): Int = when {
 ...
 */
internal fun MultiArray<*, *>.actualAxis(axis: Int): Int = if (axis < 0) dim.d + axis else axis
 ...
 */
fun IntArray.remove(pos: Int): IntArray = when (pos) {
 ...
 */
fun IntArray.removeAll(indices: List<Int>): IntArray = when {
