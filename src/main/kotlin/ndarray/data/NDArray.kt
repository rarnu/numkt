...
import com.rarnu.numkt.ndarray.operations.concatenate
 ...
 * With the help of [offset], [shape], [strides] there is a multidimensional representation
 ...
 * @param D dimension.
 */
class NDArray<T, D : Dimension>(data: ImmutableMemoryView<T>, override val offset: Int = 0, override val shape: IntArray, override val strides: IntArray = computeStrides(shape), override val dim: D, override val base: MultiArray<T, out Dimension>? = null) : MutableMultiArray<T, D> {

    init {
        check(shape.isNotEmpty()) { "Shape can't be empty." }
    }

    override val data: MemoryView<T> = data as MemoryView<T>

    override val size: Int get() = shape.fold(1, Int::times)

    override val consistent: Boolean get() = offset == 0 && size == data.size && strides.contentEquals(computeStrides(shape))

    override val indices: IntRange get() = 0 until size
    ...
    override operator fun iterator(): Iterator<T> = if (consistent) this.data.iterator() else NDArrayIterator(data, offset, strides, shape)

    inline fun <reified E : Number> asType(): NDArray<E, D> = this.asType(DataType.ofKClass(E::class))
    ...
    override fun copy(): NDArray<T, D> = NDArray(this.data.copyOf(), this.offset, this.shape.copyOf(), this.strides.copyOf(), this.dim)

    override fun deepCopy(): NDArray<T, D> {
        val data = if (consistent) {
            this.data.copyOf()
        } else {
            val tmpData = initMemoryView<T>(this.size, this.dtype)
            var index = 0
            for (el in this) {
                tmpData[index++] = el
            }
    ...
    override fun reshape(dim1: Int, dim2: Int, dim3: Int, dim4: Int, vararg dims: Int): NDArray<T, DN> {
        ...
        newShape.forEach { requirePositiveShape(it) }
        require(newShape.fold(1, Int::times) == size) { "Cannot reshape array of size $size into a new shape ${newShape.joinToString(prefix = "(", postfix = ")")}" }
    ...
    override fun transpose(vararg axes: Int): NDArray<T, D> {
        ...
        for (axis in axes) {
            require(axis in 0 until dim.d) { "Dimension must be from 0 to ${dim.d}." }
        }
    ...
    override fun squeeze(vararg axes: Int): NDArray<T, DN> {
        ...
        val newShape = this.shape.sliceArray(this.shape.indices - cutAxes.toSet())
    ...
    override infix fun cat(other: MultiArray<T, D>): NDArray<T, D> = cat(listOf(other), 0)

    override fun cat(other: MultiArray<T, D>, axis: Int): NDArray<T, D> = cat(listOf(other), axis)
    ...
    override fun cat(other: List<MultiArray<T, D>>, axis: Int): NDArray<T, D> {
        ...
        val arr = other.first()
        require(this.shape.withIndex().all { it.index == axis || it.value == arr.shape[it.index] }) { "All dimensions of input arrays for the concatenation axis must match exactly." }
    ...
    fun asD1Array(): D1Array<T> = if (this.dim.d == 1) this as D1Array<T> else throw ClassCastException("Cannot cast NDArray of dimension ${this.dim.d} to NDArray of dimension 1.")

    fun asD2Array(): D2Array<T> = if (this.dim.d == 2) this as D2Array<T> else throw ClassCastException("Cannot cast NDArray of dimension ${this.dim.d} to NDArray of dimension 2.")

    fun asD3Array(): D3Array<T> = if (this.dim.d == 3) this as D3Array<T> else throw ClassCastException("Cannot cast NDArray of dimension ${this.dim.d} to NDArray of dimension 3.")

    fun asD4Array(): D4Array<T> = if (this.dim.d == 4) this as D4Array<T> else throw ClassCastException("Cannot cast NDArray of dimension ${this.dim.d} to NDArray of dimension 4.")
        ...
        2 -> buildString {
            ...
            for (ax0 in 0 until shape[0]) {
                ...
                for (ax1 in 0 until shape[1]) {
                    ...
                    if (ax1 < shape[1] - 1) append(", ")
                }
                append(']')
                if (ax0 < shape[0] - 1) append(",\n")
            }
            append(']')
        }
