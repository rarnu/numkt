...
fun <T, D : Dimension> MultiArray<T, D>.append(arr: MultiArray<T, D>, axis: Int): NDArray<T, D> = this.cat(arr, axis)

@JvmName("stackD1")
fun <T> Numkt.stack(vararg arr: MultiArray<T, D1>, axis: Int = 0): NDArray<T, D2> = stack(arr.toList(), axis)

@JvmName("stackD1")
fun <T> Numkt.stack(arrays: List<MultiArray<T, D1>>, axis: Int = 0): NDArray<T, D2> = stackArrays(arrays, axis)

@JvmName("stackD2")
fun <T> Numkt.stack(vararg arr: MultiArray<T, D2>, axis: Int = 0): NDArray<T, D3> = stack(arr.toList(), axis)

@JvmName("stackD2")
fun <T> Numkt.stack(arrays: List<MultiArray<T, D2>>, axis: Int = 0): NDArray<T, D3> = stackArrays(arrays, axis)

@JvmName("stackD3")
fun <T> Numkt.stack(vararg arr: MultiArray<T, D3>, axis: Int = 0): NDArray<T, D4> = stack(arr.toList(), axis)

@JvmName("stackD3")
fun <T> Numkt.stack(arrays: List<MultiArray<T, D3>>, axis: Int = 0): NDArray<T, D4> = stackArrays(arrays, axis)

@JvmName("stackD4")
fun <T> Numkt.stack(vararg arr: MultiArray<T, D4>, axis: Int = 0): NDArray<T, DN> = stack(arr.toList(), axis)

@JvmName("stackD4")
fun <T> Numkt.stack(arrays: List<MultiArray<T, D4>>, axis: Int = 0): NDArray<T, DN> = stackArrays(arrays, axis)
...
fun <T, D : Dimension> MultiArray<T, D>.repeat(n: Int): D1Array<T> {
    ...
    for (i in size until (size * n) step size) {
        ...
        when (this.dtype) {
            DataType.ComplexFloatDataType -> data.getFloatArray().copyInto(data.getFloatArray(), i * 2, startIndexComplex, endIndexComplex)
            DataType.ComplexDoubleDataType -> data.getDoubleArray().copyInto(data.getDoubleArray(), i * 2, startIndexComplex, endIndexComplex)
...
internal fun <T, D : Dimension, O : Dimension> concatenate(arrays: List<MultiArray<T, D>>, dest: NDArray<T, O>, axis: Int = 0): NDArray<T, O> {
    if (axis == 0) {
        var offset = 0
...
/**
 * Clips the values in ndarray if value is not in range min to max
 *
 ...
 * @return NDArray of which all of its elements are in range min to max
 * @throws IllegalArgumentException if min > max
 */
fun <T, D : Dimension> MultiArray<T, D>.clip(min: T, max: T): NDArray<T, D> where T : Comparable<T>, T : Number {
    require(min <= max) { "min value for clipping should be lower than or equal to the [max] value" }
...
fun <T> MultiArray<T, D4>.expandDims(axis: Int): MultiArray<T, DN> = this.unsqueeze()
...
fun <T, D : Dimension> MultiArray<T, D>.expandNDims(vararg axes: Int): MultiArray<T, DN> = this.unsqueeze()
