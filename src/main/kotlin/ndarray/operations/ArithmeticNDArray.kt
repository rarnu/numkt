 ...
 */
operator fun <T, D : Dimension> MultiArray<T, D>.unaryMinus(): NDArray<T, D> = when (dtype) {
...
/**
 * Divide each element of the multidimensional array in place by another element [other].
 *
 ...
 */
internal inline fun <T : Any, D : Dimension> MutableMultiArray<T, D>.commonAssignOp(other: Iterator<T>, op: (T, T) -> T) {
    if (this.consistent) {
