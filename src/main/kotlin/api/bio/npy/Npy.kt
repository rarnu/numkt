...
package com.rarnu.numkt.api.bio.npy
...
object NpyFile {
     ...
     * header size.
     */
    internal data class Header(val order: ByteOrder? = null, val type: Char, val bytes: Int, val shape: IntArray) {
        ...
        init {
            val metaUnpadded = StringJoiner(", ", "{", "}").add("'descr': '${order.toChar()}$type$bytes'").add("'fortran_order': False").add("'shape': (${shape.joinToString(",")}, )").toString()
            ...
            val totalUnpadded = MAGIC.size + 2 + java.lang.Short.BYTES + metaUnpadded.length + 1
        ...
        override fun equals(other: Any?) = when {
            ...
            else -> order == other.order && type == other.type && bytes == other.bytes && shape.contentEquals(other.shape)
        }

        override fun hashCode() = Objects.hash(order, type, bytes, shape.contentHashCode())
        ...
        companion object {
            ...
            fun read(input: ByteBuffer) = with(input.order(ByteOrder.LITTLE_ENDIAN)) {
                ...
                check(!(meta["fortran_order"] as Boolean)) { "Fortran-contiguous arrays are not supported" }
        ...
        return FileChannel.open(path).use {
            ...
            read(generateSequence {
                ...
                if (remaining == 0L) {
                    ...
                    chunk = it.map(MapMode.READ_ONLY, offset, if (remaining > step) step.toLong() else remaining)
    ...
    @JvmOverloads
    @JvmStatic
    fun write(path: Path, data: BooleanArray, shape: IntArray = intArrayOf(data.size)) {
        write(path, allocate(data, shape))
    }

    @JvmOverloads
    @JvmStatic
    fun write(path: Path, data: ByteArray, shape: IntArray = intArrayOf(data.size)) {
        write(path, allocate(data, shape))
    }

    @JvmOverloads
    @JvmStatic
    fun write(path: Path, data: ShortArray, shape: IntArray = intArrayOf(data.size), order: ByteOrder = ByteOrder.nativeOrder()) {
        write(path, allocate(data, shape, order))
    }

    @JvmOverloads
    @JvmStatic
    fun write(path: Path, data: IntArray, shape: IntArray = intArrayOf(data.size), order: ByteOrder = ByteOrder.nativeOrder()) {
        write(path, allocate(data, shape, order))
    }

    @JvmOverloads
    @JvmStatic
    fun write(path: Path, data: LongArray, shape: IntArray = intArrayOf(data.size), order: ByteOrder = ByteOrder.nativeOrder()) {
        write(path, allocate(data, shape, order))
    }

    @JvmOverloads
    @JvmStatic
    fun write(path: Path, data: FloatArray, shape: IntArray = intArrayOf(data.size), order: ByteOrder = ByteOrder.nativeOrder()) {
        write(path, allocate(data, shape, order))
    }

    @JvmOverloads
    @JvmStatic
    fun write(path: Path, data: DoubleArray, shape: IntArray = intArrayOf(data.size), order: ByteOrder = ByteOrder.nativeOrder()) {
        write(path, allocate(data, shape, order))
    }

    @JvmOverloads
    @JvmStatic
    fun write(path: Path, data: Array<String>, shape: IntArray = intArrayOf(data.size)) {
        write(path, allocate(data, shape))
    }

    private fun write(path: Path, chunks: Sequence<ByteBuffer>) {
        FileChannel.open(path, StandardOpenOption.WRITE, StandardOpenOption.CREATE).use {
            for (chunk in chunks) {
    ...
    internal fun allocate(data: ShortArray, shape: IntArray, order: ByteOrder): Sequence<ByteBuffer> {
        val header = Header(order = order, type = 'i', bytes = java.lang.Short.BYTES, shape = shape)
        return sequenceOf(header.allocate()) + ShortArrayChunker(data, order)
    }

    internal fun allocate(data: IntArray, shape: IntArray, order: ByteOrder): Sequence<ByteBuffer> {
        val header = Header(order = order, type = 'i', bytes = Integer.BYTES, shape = shape)
        return sequenceOf(header.allocate()) + IntArrayChunker(data, order)
    }

    internal fun allocate(data: LongArray, shape: IntArray, order: ByteOrder): Sequence<ByteBuffer> {
        val header = Header(order = order, type = 'i', bytes = java.lang.Long.BYTES, shape = shape)
        return sequenceOf(header.allocate()) + LongArrayChunker(data, order)
    }

    internal fun allocate(data: FloatArray, shape: IntArray, order: ByteOrder): Sequence<ByteBuffer> {
        val header = Header(order = order, type = 'f', bytes = java.lang.Float.BYTES, shape = shape)
        return sequenceOf(header.allocate()) + FloatArrayChunker(data, order)
    }

    internal fun allocate(data: DoubleArray, shape: IntArray, order: ByteOrder): Sequence<ByteBuffer> {
        val header = Header(order = order, type = 'f', bytes = java.lang.Double.BYTES, shape = shape)
...
class NpyArray(
    ...
    override fun toString() = StringJoiner(", ", "NpyArray{", "}").add("data=" + arrayOf(data).contentDeepToString().removeSurrounding("[", "]")).add("shape=" + shape.contentToString()).toString()
}
