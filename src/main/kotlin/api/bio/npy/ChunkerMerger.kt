...
internal abstract class ArrayChunker<T>(
    ...
    override fun iterator() = object : Iterator<ByteBuffer> {
        ...
        override fun next(): ByteBuffer {
            ...
            val result = if (available == step) cache.apply { rewind() } else ByteBuffer.allocateDirect(available * bytes).order(order)
            with(result) {
...
internal class BooleanArrayChunker(data: BooleanArray) : ArrayChunker<BooleanArray>(data, data.size, ByteOrder.nativeOrder()) {
...
internal class ShortArrayChunker(data: ShortArray, order: ByteOrder) : ArrayChunker<ShortArray>(data, data.size, order) {
...
internal class IntArrayChunker(data: IntArray, order: ByteOrder) : ArrayChunker<IntArray>(data, data.size, order) {
...
internal class LongArrayChunker(data: LongArray, order: ByteOrder) : ArrayChunker<LongArray>(data, data.size, order) {
...
internal class FloatArrayChunker(data: FloatArray, order: ByteOrder) : ArrayChunker<FloatArray>(data, data.size, order) {
...
internal class DoubleArrayChunker(data: DoubleArray, order: ByteOrder) : ArrayChunker<DoubleArray>(data, data.size, order) {
...
internal class StringArrayChunker(data: Array<String>) : ArrayChunker<Array<String>>(data, data.size, ByteOrder.nativeOrder()) {
...
internal class BooleanArrayMerger(size: Int) : ArrayMerger<BooleanArray>(BooleanArray(size)) {
...
internal class StringArrayMerger(size: Int, private val bytes: Int) : ArrayMerger<Array<String>>(Array(size) { "" }) {
