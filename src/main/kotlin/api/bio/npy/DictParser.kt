...
/** This is a very basic parser for repr of Python dict. */
internal fun parseDict(s: String): Map<String, Any> = parseDictInternal(tokenize(s))
...
private fun tokenize(s: String): PeekingIterator<SpannedToken> {
    ...
    val tokens = generateSequence {
        val best = Token.entries.mapNotNull { token ->
            token.regex.find(leftover)?.let { SpannedToken(token, it.value) }
        }.maxByOrNull { it.span.length }

        if (best != null) {
            leftover = leftover.substring(best.span.length).dropWhile(Char::isWhitespace)
        }
...
private fun PeekingIterator<SpannedToken>.tryEat(expected: Token): SpannedToken? = if (hasNext() && peek().token == expected) next() else null

// doesn't support iterators which might yield null.
internal class PeekingIterator<out T>(private val it: Iterator<T>) : Iterator<T> {
    ...
    override fun next(): T = if (hasPeeked) {
        val result = item
        hasPeeked = false
        item = null
        result!!
