...
internal fun NDArray<ComplexFloat, D2>.conjTranspose(): D2Array<ComplexFloat> {
    val ans = Numkt.zeros<ComplexFloat>(this.shape[1], this.shape[0])
...
internal fun NDArray<ComplexDouble, D2>.conjTranspose(): D2Array<ComplexDouble> {
    val ans = Numkt.zeros<ComplexDouble>(this.shape[1], this.shape[0])
    for (i in 0 until ans.shape[0]) {
        for (j in 0 until ans.shape[1]) {
            ans[i, j] = this[j, i].conjugate()
        }
...
internal fun requireDotShape(aShape: IntArray, bShape: IntArray) = require(aShape[1] == bShape[0]) {
    "Shapes mismatch: shapes " + "${aShape.joinToString(prefix = "(", postfix = ")")} and " + "${bShape.joinToString(prefix = "(", postfix = ")")} not aligned: " + "${aShape[1]} (dim 1) != ${bShape[0]} (dim 0)"
}
 ...
 */
internal inline fun swapLines(rowPerm: MultiArray<Int, D1>, from1: Int = 0, to1: Int = rowPerm.size, from2: Int = 0, to2: Int, swap: (Int, Int) -> Unit) {
