...
class NpyFileTest(private val order: ByteOrder) {
    ...
    @Test
    fun writeReadBooleans() = withTempFile("test", ".npy") { path ->
    ...
    @Test
    fun writeReadBytes() = withTempFile("test", ".npy") { path ->
    ...
    @Test
    fun writeReadShorts() = withTempFile("test", ".npy") { path ->
    ...
    @Test
    fun writeReadInts() = withTempFile("test", ".npy") { path ->
    ...
    @Test
    fun writeReadLongs() = withTempFile("test", ".npy") { path ->
    ...
    @Test
    fun writeReadFloats() = withTempFile("test", ".npy") { path ->
    ...
    @Test
    fun writeReadDoubles() = withTempFile("test", ".npy") { path ->
    ...
    @Test
    fun writeReadStrings() = withTempFile("test", ".npy") { path ->
    ...
    companion object {
        ...
        @JvmStatic
        @Parameters(name = "{0}")
        fun `data`(): Collection<Any> = listOf(ByteOrder.BIG_ENDIAN, ByteOrder.LITTLE_ENDIAN)
    }
}
...
class NpyFileNDTest(private val shape: IntArray) {
    ...
    @Test
    fun writeReadDouble() = withTempFile("test", ".npy") { path ->
    ...
    companion object {
        ...
        @JvmStatic
        @Parameters
        fun `data`() = listOf(intArrayOf(1, 8), intArrayOf(8, 1), intArrayOf(2, 4), intArrayOf(4, 2), intArrayOf(2, 2, 2), intArrayOf(2, 2, 2, 1))
    }
}
...
class NpyFileHeaderTest {
    ...
    @Test
    fun writeRead10() = testWriteRead(65536)

    @Test
    fun writeRead20() = testWriteRead(16)
...
class NpyFileNumPyTest {
    ...
    fun writeRead() {
        ...
        withTempFile("test", ".npy") { path ->
            val data = intArrayOf(1, 2, 3, 4)
            NpyFile.write(path, data)
            val (rc, output) = command("python", "-c", "import numpy as np; print(np.load('$path'))")
            assertEquals(0, rc)
    ...
    private fun command(vararg args: String): Pair<Int, String> {
        val p = ProcessBuilder().command(*args).redirectOutput(Redirect.PIPE).start()
...
class NpyFileStressTest {
    ...
    @Test
    fun writeReadUnaligned() = withTempFile("test", ".npy") { path ->
        val data = Random().doubles(65536).toArray()
        NpyFile.write(path, data)
        assertArrayEquals(data, NpyFile.read(path, step = 123).asDoubleArray(), Math.ulp(1.0))
    }
}
...
class NpyFileNonRegressionTest {
    ...
    @Test
    fun readLongShapePython2() {
        assertArrayEquals(doubleArrayOf(1.0, 1.0), NpyFile.read(Examples["win64python2.npy"]).asDoubleArray(), Math.ulp(1.0))
    }
}
