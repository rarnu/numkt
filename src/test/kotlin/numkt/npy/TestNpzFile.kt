...
class NpzFileTest {
    ...
    fun introspectTypes() {
        NpzFile.read(Examples["example.npz"]).use { npzf ->
            assertEquals(
                mapOf("x_i1" to Byte::class.java, "x_i2" to Short::class.java, "x_u4" to Int::class.java, "x_i8" to Long::class.java, "x_f4" to Float::class.java, "x_f8" to Double::class.java, "x_b" to Boolean::class.java, "x_S3" to String::class.java),
    ...
    fun introspectNames() {
        NpzFile.read(Examples["example.npz"]).use { npzf ->
            assertEquals(setOf("x_i1", "x_i2", "x_u4", "x_i8", "x_f4", "x_f8", "x_b", "x_S3"), npzf.introspect().map { it.name }.toSet())
        }
    }
    ...
    fun get() {
        NpzFile.read(Examples["example.npz"]).use { npzf ->
            ...
            assertArrayEquals(floatArrayOf(1.0f, 2.0f, 3.0f, 4.0f), npzf["x_f4"].asFloatArray(), Math.ulp(1.0f))
            assertArrayEquals(doubleArrayOf(1.0, 2.0, 3.0, 4.0), npzf["x_f8"].asDoubleArray(), Math.ulp(1.0))
            assertArrayEquals(booleanArrayOf(true, true, true, false), npzf["x_b"].asBooleanArray())
...
class NpzFileWriteReadTest(private val order: ByteOrder) {
    ...
    fun nested() {
        withTempFile("test", ".npz") { path ->
            ...
            NpzFile.read(path).use { npzf ->
                assertArrayEquals(intArrayOf(1, 2, 3, 4), npzf["foo/bar/baz/x_i4"].asIntArray())
            }
        }
    }
    ...
    fun nd() {
        withTempFile("test", ".npz") { path ->
            NpzFile.write(path).use {
                it.write("x_i4", intArrayOf(1, 2, 3, 4), shape = intArrayOf(2, 2), order = order)
            }
    ...
    fun random() {
        ...
        for (i in 0 until 10) {
            val data = Array(r.nextInt(maxSize)) {
                ...
                val alphabet = "ACGT"
                val b = ByteArray(r.nextInt(12) + 1) { alphabet[r.nextInt(4)].code.toByte() }
                String(b)
            }
