...
package com.rarnu.numkt.test.kotlin.linalg
...
class KELinAlgTest {
    ...
    @Test
    fun testOfNormFunctionWithPEquals1() {
    ...
    fun testKEDot() {
        ...
        val mat1XMat1 = Numkt.ndarray(Numkt[Numkt[40, 27, 39], Numkt[9, 57, 16], Numkt[-9, -33, -11]])
        val mat1XMat2 = Numkt.ndarray(Numkt[Numkt[-6, 26, -36], Numkt[105, -98, -55], Numkt[-60, 64, 47]])
        val mat1XMat3 = Numkt.ndarray(Numkt[Numkt[-7, 1, -5], Numkt[-48, 156, 29], Numkt[21, -105, -19]])
        val mat1XMat4 = Numkt.ndarray(Numkt[Numkt[-4, 2, -7], Numkt[67, -91, 120], Numkt[-35, 50, -69]])

        val mat2XMat2 = Numkt.ndarray(Numkt[Numkt[25, -92, 40], Numkt[74, 20, 118], Numkt[-70, 84, 73]])
        val mat2XMat3 = Numkt.ndarray(Numkt[Numkt[28, 54, 21], Numkt[-104, -56, -6], Numkt[17, -141, -25]])
        val mat2XMat4 = Numkt.ndarray(Numkt[Numkt[8, -9, 30], Numkt[82, -86, 80], Numkt[-37, 56, -81]])

        val mat3XMat3 = Numkt.ndarray(Numkt[Numkt[59, 11, -4], Numkt[2, 76, 4], Numkt[-117, 117, 22]])
        val mat3XMat4 = Numkt.ndarray(Numkt[Numkt[-57, 61, -65], Numkt[-7, -4, 8], Numkt[116, -143, 165]])

        val mat1XVec1 = Numkt.ndarray(Numkt[35, -63, 39])
        val mat1XVec2 = Numkt.ndarray(Numkt[49, 44, -34])
        val mat2XVec1 = Numkt.ndarray(Numkt[-97, -6, 49])
        val mat2XVec2 = Numkt.ndarray(Numkt[-89, -52, -50])
        ...
        //Start test cases
        //Byte
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toByte() }, mat1.map { it.toByte() }).data, mat1XMat1.map { it.toByte() }.data)
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toByte() }, mat2.map { it.toByte() }).data, mat1XMat2.map { it.toByte() }.data)
        assertEquals(KELinAlgEx.dotMV(mat1.map { it.toByte() }, vec1.map { it.toByte() }).data, mat1XVec1.map { it.toByte() }.data)
        assertEquals(KELinAlgEx.dotVV(vec1.map { it.toByte() }, vec2.map { it.toByte() }), vec1XVec2.toByte())

        //Short
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toShort() }, mat1.map { it.toShort() }).data, mat1XMat1.map { it.toShort() }.data)
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toShort() }, mat2.map { it.toShort() }).data, mat1XMat2.map { it.toShort() }.data)
        assertEquals(KELinAlgEx.dotMV(mat1.map { it.toShort() }, vec1.map { it.toShort() }).data, mat1XVec1.map { it.toShort() }.data)
        assertEquals(KELinAlgEx.dotVV(vec1.map { it.toShort() }, vec2.map { it.toShort() }), vec1XVec2.toShort())

        //int
        assertEquals(KELinAlgEx.dotMM(mat1, mat1).data, mat1XMat1.data)
        assertEquals(KELinAlgEx.dotMM(mat1, mat2).data, mat1XMat2.data)
        assertEquals(KELinAlgEx.dotMV(mat1, vec1).data, mat1XVec1.data)
        assertEquals(KELinAlgEx.dotVV(vec1, vec2), vec1XVec2)

        //Long
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toLong() }, mat1.map { it.toLong() }).data, mat1XMat1.map { it.toLong() }.data)
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toLong() }, mat2.map { it.toLong() }).data, mat1XMat2.map { it.toLong() }.data)
        assertEquals(KELinAlgEx.dotMV(mat1.map { it.toLong() }, vec1.map { it.toLong() }).data, mat1XVec1.map { it.toLong() }.data)
        assertEquals(KELinAlgEx.dotVV(vec1.map { it.toLong() }, vec2.map { it.toLong() }), vec1XVec2.toLong())

        //Float
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toFloat() }, mat1.map { it.toFloat() }).data, mat1XMat1.map { it.toFloat() }.data)
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toFloat() }, mat2.map { it.toFloat() }).data, mat1XMat2.map { it.toFloat() }.data)
        assertEquals(KELinAlgEx.dotMV(mat1.map { it.toFloat() }, vec1.map { it.toFloat() }).data, mat1XVec1.map { it.toFloat() }.data)
        assertEquals(KELinAlgEx.dotVV(vec1.map { it.toFloat() }, vec2.map { it.toFloat() }), vec1XVec2.toFloat())

        //Double
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toDouble() }, mat1.map { it.toDouble() }).data, mat1XMat1.map { it.toDouble() }.data)
        assertEquals(KELinAlgEx.dotMM(mat1.map { it.toDouble() }, mat2.map { it.toDouble() }).data, mat1XMat2.map { it.toDouble() }.data)
        assertEquals(KELinAlgEx.dotMV(mat1.map { it.toDouble() }, vec1.map { it.toDouble() }).data, mat1XVec1.map { it.toDouble() }.data)

        //ComplexDouble
        assertEquals(KELinAlgEx.dotMMComplex(composeComplexDouble(mat1, mat2), composeComplexDouble(mat3, mat4)).data, composeComplexDouble(mat1XMat3 - mat2XMat4, mat1XMat4 + mat2XMat3).data)
        assertEquals(KELinAlgEx.dotMVComplex(composeComplexDouble(mat1, mat2), composeComplexDouble(vec1, vec2)).data, composeComplexDouble(mat1XVec1 - mat2XVec2, mat1XVec2 + mat2XVec1).data)
        assertEquals(KELinAlgEx.dotVVComplex(composeComplexDouble(vec1, vec2), composeComplexDouble(vec3, vec4)), ComplexDouble(vec1XVec3 - vec2XVec4, vec1XVec4 + vec2XVec3))

        //ComplexFloat
        assertEquals(KELinAlgEx.dotMMComplex(composeComplexFloat(mat1, mat2), composeComplexFloat(mat3, mat4)).data, composeComplexFloat(mat1XMat3 - mat2XMat4, mat1XMat4 + mat2XMat3).data)
        assertEquals(KELinAlgEx.dotMVComplex(composeComplexFloat(mat1, mat2), composeComplexFloat(vec1, vec2)).data, composeComplexFloat(mat1XVec1 - mat2XVec2, mat1XVec2 + mat2XVec1).data)
        assertEquals(KELinAlgEx.dotVVComplex(composeComplexFloat(vec1, vec2), composeComplexFloat(vec3, vec4)), ComplexFloat(vec1XVec3 - vec2XVec4, vec1XVec4 + vec2XVec3))
    }

    @Test
    fun testUpperHessenbergForm() {
        val n = 300
        ...
        val i = Numkt.zeros<ComplexDouble>(n, n)
...
private fun <T : Number, D : Dim2> assertClose(a: MultiArray<T, D>, b: MultiArray<T, D>, precision: Double) {
    ...
    if (a.dim.d == 1) {
        ...
        b as D1Array<T>
        for (i in 0 until a.size) {
            maxabs = max(abs(a[i].toDouble() - b[i].toDouble()), maxabs)
        }
    } else {
        a as D2Array<T>
        b as D2Array<T>
